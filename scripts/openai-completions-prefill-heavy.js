...
export default function run() {
  ...
  const template = any(longContextTemplates);
  const contextTemplate = template.context;
  ...
  const messages = [
    ...
    { role: 'user', content: contextTemplate + "\n\nBased on the above information, " + mainQuestion }
  ];
  ...
  if (conversationTurns > 1) {
    ...
    for (let i = 1; i < conversationTurns; i++) {
      ...
      messages.push({ 
        role: 'assistant', 
        content: 'This is a simulated previous response to maintain the conversation history and increase the context size.' 
      });
      
      // Add a follow-up question
      messages.push({ 
        role: 'user', 
        content: any(followUpQuestions) 
      });
    }
  }
  ...
  console.log(`Prefill test with ${contextSize} chars context, ${conversationTurns} conversation turns`);
